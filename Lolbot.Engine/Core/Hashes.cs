...
{
    // Seeds are defined as per http://hgm.nubati.net/book_format.html
    ...
    const int CastleOffset = 768;
    const int EnPassantOffset = 772;
    const int ColorOffset = 780;
    private readonly static ulong[] Seeds = [
        #region Polyglot Seeds
    ...
    public static ulong New(MutablePosition position)
    {
        ...
        {
            var bitboard = position[piece];
            while (bitboard != 0)
            {
                var sq = Bitboards.PopLsb(ref bitboard);
                zobrist ^= GetValue(piece, sq);
            }
        }
        ...
        zobrist ^= GetValue(pawnAttackMap, position.EnPassant);
        zobrist ^= GetValue(position.CurrentPlayer);
    ...
    public static ulong GetValue(Piece piece, byte square)
    {
        ...
        return Seeds[(64 * pieceValue) + square];
    }
}