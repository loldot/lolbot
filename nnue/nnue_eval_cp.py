#!/usr/bin/env python3
"""
NNUE Evaluation Script with Centipawn Output
Reads FEN strings from stdin and outputs evaluations in centipawns.
"""
...
def wdl_to_centipawns(wdl: float) -> int:
    """
    Convert WDL probability to centipawns using logistic mapping.
    WDL = 0.0 means BLACK wins, WDL = 1.0 means WHITE wins.
    """
    # Clamp to avoid extreme values
    wdl = max(0.001, min(0.999, wdl))
    
    # WDL is already white advantage (1.0 = white wins, 0.0 = black wins)
    white_prob = wdl
    
    # Use logistic inverse to convert probability to centipawns
    # This is approximately how chess engines map evaluations
    if white_prob >= 0.5:
        # White is better
        logit = math.log(white_prob / (1 - white_prob))
        centipawns = int(logit * 100)
    else:
        # Black is better
        logit = math.log((1 - white_prob) / white_prob)
        centipawns = int(-logit * 100)
    
    # Cap at reasonable values
    return max(-3000, min(3000, centipawns))
...
def evaluate_fen(model: NNUE, fen: str, output_format: str = "cp") -> str:
    ...
    try:
        ...
        if output_format == "wdl":
            return f"{wdl_value:.6f}"
        else:  # centipawns
            cp = wdl_to_centipawns(wdl_value)
            return f"{cp:+d}"
...
def main():
    ...
    output_format = "cp"  # default to centipawns
    if len(sys.argv) > 1 and sys.argv[1] == "--wdl":
        output_format = "wdl"
    ...
    if not os.path.exists(model_path):
        print(f"Error: Model file not found: {model_path}", file=sys.stderr)
        print("Please train a model first using train_improved.py", file=sys.stderr)
        sys.exit(1)
    ...
    if output_format == "wdl":
        print("Output format: WDL probabilities (0.0=black wins, 1.0=white wins)", file=sys.stderr)
    else:
        print("Output format: Centipawns (+N=white better, -N=black better)", file=sys.stderr)
    print("Usage: echo 'FEN' | python nnue_eval_cp.py [--wdl]", file=sys.stderr)
    ...
    try:
        for line in sys.stdin:
            ...
            evaluation = evaluate_fen(model, fen, output_format)
