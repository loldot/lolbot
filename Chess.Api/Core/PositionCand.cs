...
public readonly record struct PositionCand
{
    ...
    public static PositionCand EmptyBoard => new PositionCand() with
    {
        WhitePawns = 0,
        WhiteKnights = 0,
        WhiteBishops = 0,
    ...
    public PositionCand Move(Move m)
    {
        ...
        if (m.CastleIndex != 0)
        {
            var w_castle = Castle(Bitboards.Masks.Rank_1 & ~(Bitboards.Masks.A_File | Bitboards.Masks.H_File), m);
            var b_castle = Castle(Bitboards.Masks.Rank_8 & ~(Bitboards.Masks.A_File | Bitboards.Masks.H_File), m);
    ...
    public PositionCand Update(Piece piece, ulong bitboard) => piece switch
    {
